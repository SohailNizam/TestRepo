...
#' fits. If \code{NULL}, no averaging is performed (in which \code{n_SL} 
...
drtmle <- function(Y, A, W,
  ...
  # Instantiate empty lists (length = 1 if n_SL = 1 or "drtmle" not in avg_over):
  #   nuisance_drtmle 
  #   nuisance_aiptw_c
  #   ic_drtmle 
  ...
  # For introducing cross-validated standard errors if se_cv = "full", 
  # but cvFolds = 1
  ...
  for(i in seq_len(n_loops)){
    ...
    if (is.null(Qn)) {
      ...
      # TO DO: MAKE SURE THIS IS WORKING AS EXPECTED!!!
